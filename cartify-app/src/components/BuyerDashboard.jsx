...
const ProfileSection = ({ userProfile, onNotificationToggle }) => {
  ...
  useEffect(() => {
    const fetchReferralData = async () => {
      try {
        ...
        // Replace with your actual referral API endpoint
        const response = await fetch('https://carttifys-1.onrender.com/api/referrals/my-stats', {
          headers: {
        ...
        if (response.ok) {
          ...
          if (result.success) {
            ...
            // If no referral data, set default structure
            setReferralData({
  ...
  const generateReferralCode = () => {
    ...
    return `${userId.toUpperCase()}${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
  };

  const copyReferralLink = () => {
    const referralLink = `https://your-app.com/signup?ref=${referralData.referralCode}`;
  ...
  const shareReferral = () => {
    const referralLink = `https://your-app.com/signup?ref=${referralData.referralCode}`;
    const shareText = `Join me on Carttify! Use my referral code ${referralData.referralCode} to get started. ${referralLink}`;
    
    if (navigator.share) {
      navigator.share({
        title: 'Join Carttify Marketplace',
        text: shareText,
        url: referralLink
      });
    } else {
  ...
  if (!userProfile.name || loadingReferral) {
    return (
      ...
      <div className="referral-section">
        ...
        <div className="referral-stats">
          ...
          <div className="referral-stat-card">
            ...
            <div className="stat-info">
              <h4>{referralData?.successfulReferrals || 0}</h4>
              <p>Successful</p>
            </div>
          </div>
          ...
          <div className="referral-stat-card">
            ...
            <div className="stat-info">
              <h4>${referralData?.totalEarned || 0}</h4>
              <p>Total Earned</p>
            </div>
          </div>
        </div>
        ...
        <div className="referral-code-section">
          ...
          <div className="referral-link">
            ...
            <div className="link-display">
              <span className="link">https://your-app.com/signup?ref={referralData?.referralCode}</span>
        ...
        <div className="referral-benefits">
          ...
          <ul>
            <li>✓ Get $10 credit for each friend who signs up</li>
            <li>✓ Your friend gets $5 off their first order</li>
            <li>✓ Credits are applied after friend's first purchase</li>
              ...
              {referralData.referrals.slice(0, 5).map((referral, index) => (
                <div key={index} className="referral-item">
                  <div className="referral-info">
                    <span className="referral-name">{referral.friendName || 'Friend'}</span>
                    <span className="referral-status">{referral.status}</span>
                  </div>
                  <span className="referral-bonus">+${referral.bonus || 0}</span>
                </div>
              ))}
            </div>
          </div>
